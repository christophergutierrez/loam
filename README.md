...
| [ADRs](./docs/adr/) | Settled decisions that refine the PRDs (ADR-0001…0007): census-by-default, v1 independence from any external telemetry consumer, the rediscovery premise as measured hypothesis, P0 gate weighting, the backend-agnostic endpoint abstraction, the Rust-demand / Python-supply split across an artifact seam, and the P0-gate override to build `loam-core`. Where an ADR and older PRD prose disagree, the ADR wins. |
...
Both PRDs are at v0.1 (draft), the glossary and the **P0 Experiment Protocol** are drafted, and seven ADRs (0001–0007) refine them. `loam-core` is under active build (ADR-0007). The immediate next step is executing P0: label the ~100-file sample and run the model-floor experiment that tests kill criterion 1 before anything else is built — now reporting, as promoted primary readouts, the *anchor claims-per-GPU-hour* that sizes the census/sampling window (ADR-0001) and the *discovery-vs-execution token split* that sets the savings ceiling (ADR-0003). Component TRDs follow P0 results, not precede them.
