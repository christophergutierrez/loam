...
- **No external telemetry consumer in v1 (ADR-0002):** phases 1–3 emit to the local spool only; the feedback loops (heat/promotion/economics-live) are P1+ amplifiers that light up when a downstream consumer drains the spool. The treatment-arm economics (Phase 3) use the local spool + paired-run harness, independent of any downstream consumer.
...
| O-F advanced consumption (P3) | 6 | P2 + downstream consumer intake | coarse |
...
- Downstream consumer's intake contract still absent when Phase 6 starts → Phase 6 promotion/economics-live items stay parked (spool accumulates); the rest of P3 proceeds.
